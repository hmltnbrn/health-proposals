  ...
  function renderMobile(proposals) {
    ...
    d3.selectAll(".mobile-" + String(system) + "-container .mobile-small-block").each(function (d, i) {
      ...
      if (i <= proposalsLength - 1) {
        var divs = d3.select(".mobile-" + String(system) + "-container .mobile-plan-container").selectAll("div.mobile-full-view-" + String(coord)).data(proposals.filter(function (d) {
          return d.x === coord;
        }));
        divs.enter().insert("div", "." + String(d3.select(this.nextElementSibling).attr("class").split(" ").join("."))).attr("class", "mobile-full-view mobile-full-view-" + String(coord));
        divs.html(function (d) {
          return setFullTooltopHtml(d);
        }).style("display", "none");
        divs.exit().remove();
        d3.select(this).on("click", function () {
          var points = document.querySelectorAll(".mobile-full-view-" + String(coord));
          points.forEach(function (p) {
