...
(function () {
  ...
  function renderAll() {
    if (!mobile) {
      ...
      d3.selectAll(".descriptive-text .what-to-do").text("Click or tap the dots along the continuum");
    } else {
      if (system === "federal") {
        d3.select(".mobile-federal-container").show();
        d3.select(".mobile-international-container").hide();
        d3.select(".descriptive-text.federal-text .what-to-do").text("Tap on the subcategories");
        renderMobile(federalData);
      } else if (system === "international") {
        d3.select(".mobile-international-container").show();
        d3.select(".mobile-federal-container").hide();
        d3.select(".descriptive-text.international-text .what-to-do").text("Tap on the countries");
  ...
  function renderDesktop(proposals) {
    ...
    var circleAttrs = {
      cx: function cx(d) {
        if (previousCxValue === d.x) cxOffset += 100 / proposalsLength / proposalsCounted[d.x];else cxOffset = 2;
      ...
      fill: function fill(d) {
        if (previousColorValue === d.x) colorOffset += 100 / proposalsLength / proposalsCounted[d.x];else colorOffset = 2;
    ...
    function setFullTooltopHtml(d) {
      if (system === "federal") {
        ...
        return "\n          <div class=\"option-head\">\n            <h4 class=\"option-head-number\">Option " + String(checking.length) + "</h4>\n            <div class=\"close-full-tooltip\">\n              <i id=\"close-" + String(d.id) + "\" class=\"material-icons x-full-tooltip\">close</i>\n            </div>\n          </div>\n          <h2>" + String(d.name) + "</h2>\n          <h3>Sponsor</h3>\n          <p>" + String(d.sponsor) + "</p>\n          <h3>New Coverage Enhancement or Option</h3>\n          <p>" + String(d.enhancement) + "</p>\n          <h3>Who Is Eligible?</h3>\n          <p>" + String(d.eligibility) + "</p>\n          <h3>How Do People Pay for Coverage and Health Care?</h3>\n          <p>" + String(d.cost) + "</p>\n          <h3>How Are Health Care Costs Managed?</h3>\n          <p>" + String(d.management) + "</p>\n          <h3>" + String(transitionHeader) + "</h3>\n          <p>" + String(transitionText) + "</p>\n          " + String(d.links.map(function (l) {
          return "<a href=\"" + String(l.url) + "\" target=\"_blank\">Read the " + String(l.chamber) + " bill</a>";
        }).join("")) + "\n        ";
      } else if (system === "international") {
    ...
    function maintainTimelineHeight(d) {
      ...
      var newHeight = 200 * sectionActive[d.x];
  ...
  function renderMobile(proposals) {
    ...
    var systemContainer = ".mobile-" + String(system) + "-container .mobile-plan-container";
    if (system === "international") systemContainer += " .mobile-small-block-container";
    d3.selectAll(".mobile-" + String(system) + "-container .mobile-small-block").each(function (d, i) {
      var coord = parseInt(d3.select(this).attr("class").split(" ")[2].split("-")[2]);

      if (i <= proposalsLength - 1) {
        var divs = d3.select(systemContainer).selectAll("div.mobile-full-view-" + String(coord)).data(proposals.filter(function (d) {
          return d.x === coord;
        }));
        divs.enter().insert("div", "." + String(d3.select(this.nextElementSibling).attr("class").split(" ").join("."))).attr("class", "mobile-full-view-container mobile-full-view-" + String(coord));
        divs.html(function (d) {
          return setFullTooltopHtml(d);
        });
        divs.exit().remove();
        d3.select(this).on("click", function () {
          var points = document.querySelectorAll(".mobile-" + String(system) + "-container .mobile-full-view-" + String(coord));
          points.forEach(function (p) {
