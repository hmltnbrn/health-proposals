...
(function () {
  ...
  function checkSize(error, federalProposals, internationalSystems) {
    ...
    if (screen.width <= 768 || window.innerWidth <= 768) {
      mobile = true; // d3.select(".mobile-container").show();
      // d3.select(".desktop-container").hide();
    } else {
      mobile = false; // d3.select(".desktop-container").show();
  ...
  function renderDesktop(proposals) {
    ...
    function mouseOver(d) {
      if (!clicked[d.id]) {
        ...
        var title = activeFullTools >= 3 ? 'title="Only 3 comparisons allowed at once"' : "";
        ...
        var _title = activeFullTools >= 3 ? 'title="Only 3 comparisons allowed at once"' : "";
    ...
    function disableInputs(d) {
      d3.selectAll(".viz-tooltip").each(function (d, i) {
        ...
        if (checking.indexOf(parseInt(id)) === -1) {
          ...
          d3.select(this).select(".outer-span").attr("title", "Only 3 comparisons allowed at once").style("cursor", "not-allowed");
        }
      });
      reachedLimit = true;
    }
